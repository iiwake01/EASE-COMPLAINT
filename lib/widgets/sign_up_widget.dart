      ...
      child: Padding (
        ...
        child:  Column (
          ...
          children: <Widget> [
            ...
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            Obx(() => DropdownButton (value: controller.selectedGender.value, items: controller.genderList.map<DropdownMenuItem<String>>((gender) => DropdownMenuItem(value: gender,child: Text(gender),)).toList(), onChanged: (gender) => controller.updateGender(gender), )),
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            ...
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            Obx(() => DropdownButton (value: controller.selectedStatus.value, items: controller.statusList.map<DropdownMenuItem<String>>((status) => DropdownMenuItem(value: status, child: Text(status),)).toList(), onChanged: (status) => controller.updateStatus(status),),),
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            Obx(() => DropdownButton (value: controller.selectedZone.value, items: controller.zoneList.map<DropdownMenuItem<String>>((zone) => DropdownMenuItem(value: zone, child: Text(zone),)).toList(), onChanged: (zone) => controller.updateZone(zone),),),
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            ...
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
            Row( children: [ Text(AppLocalizations.of(context).translate('please_attach_any_valid_proof_of_residency'),), IconButton(onPressed: () => controller.onPickFiles(), icon: const Icon(CupertinoIcons.paperclip,)), Obx(() => Text(controller.liveFile.value))],),
            SizedBox(height: MediaQuery.of(context).size.height * 0.025,),
