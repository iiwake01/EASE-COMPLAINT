...
class FirestoreService extends GetxService {
  ...
  Future<void> updateResident(ResidentModel? model) async {
    if (model != null)
      _update("residents", model.id, model.toMap());
    else
  ...
  Future<void> updateComplaint(ComplaintModel? model) async {
    if (model != null)
      _update("complaints", model.id, model.toMap());
    else
  ...
  Future<void> updateStaff(StaffModel? model) async {
    if (model != null)
      _update("staffs", model.id, model.toMap());
    else
  ...
  Future<void> updateNotification(NotificationModel? model) async {
    if (model != null)
      _update("notifications", model.id, model.toMap());
    else
      throw Exception("NotificationModel is Null");
  }

  Future<QuerySnapshot<Map<String, dynamic>>> _get(String collectionPath,
      {String? uid}) async {
    if (uid != null) {
  ...
  Future<List<ComplaintModel>> getComplaintsStatus(
  ...
  Future<List<ComplaintModel>> getComplaintsType(
  ...
  Future<List<NotificationModel>> getNotifications(String? uid) async {
    ...
    final response = await dbFirestore
        ...
        .orderBy(Constants.DATELASTUPDATED, descending: true)
        .get();
    return response.docs
        .map((doc) => NotificationModel.fromSnapshot(doc))
        .toList();
  ...
  Future<void> deleteResident(ResidentModel? model) async {
    if (model != null)
      await _delete("residents", model.id!);
    else
      throw Exception("Error deleting Resident");
  }

  Future<void> deleteStaff(StaffModel? model) async {
    if (model != null)
      await _delete("staffs", model.id!);
    else
      throw Exception("Error deleting Staff");
  }

  Future<void> deleteComplaint(ComplaintModel? model) async {
    if (model != null)
      await _delete("complaints", model.id!);
    else
      throw Exception("Error deleting complaints");
  }
}
